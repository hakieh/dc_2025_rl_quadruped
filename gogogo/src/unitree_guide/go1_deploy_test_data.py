...
class Run():
    ...
    def policy(self, lock):
        ...
        cursor = connection.cursor()
                    ...
                    # base_ang_vel ARRAY(FLOAT),
                    # projected_gravity ARRAY(FLOAT),
                    # velocity_commands ARRAY(FLOAT),
                    # joint_pos ARRAY(FLOAT),
                    # joint_vel ARRAY(FLOAT),
                    # last_action ARRAY(FLOAT),

        cursor.execute("""
                CREATE TABLE IF NOT EXISTS deployData(
        ...
        vel_lin_xy = torch.zeros(2, device = "cuda:0")
        vel_lin_z = 0
        feet_air_time = torch.zeros(4, device = "cuda:0") 
        first_contact = torch.zeros(4, device = "cuda:0")
        last_contact = torch.ones(4, device = "cuda:0")
        pre_time=time.time()
        ...
        for i in range(int(self.control_freq*(self.run_time+10))):
            ...
            self.offset = self.Tupian.getoffset()
            # print(self.offset)

            # print(self.env.GetFootContacts())
            obs = self.env.getObservation_isaacsim_new_new()
            obs[0][8] = -self.offset/200.0
            if(abs(obs[0][8])>0.2):
                obs[0][6]=0.1
            # print(obs[0][6:9])
            # his = obs[:,:-45]
            ...
            # print(obs)
            # print()
            blend = np.clip( j /(self.control_freq*1),0,1)#blend within half a sec
            ...
            ang_acc = self.env.state.imu.accelerometer
            for i in range(2):
                vel_lin_xy[i] += ang_acc[i] * self.control_dt
            
            # print(vel_lin_xy)

            vel_lin_z += ang_acc[2] * self.control_dt

            # 旧
            last_air_time = torch.zeros(4, device = "cuda:0")
            currenttime = time.time() - self.env.starttime
            now_time=time.time()
            contact_real=self.env.GetFootContacts()
            contact = torch.tensor([contact_real[1],contact_real[0],contact_real[3],contact_real[2]],device="cuda:0")

            ########
            for i in range(4):
                if contact[i] == 0:
                    feet_air_time[i] += now_time-pre_time
                else:
                    if last_contact[i]==0:
                        last_air_time[i] = feet_air_time[i]
                        feet_air_time[i] = 0
                    else:
                        pass
            pre_time=now_time
            first_contact=(contact[i]==1)&(last_contact[i]==0)
            last_contact=contact.clone().detach()

            # print(last_air_time.size())
            # last_air_time = torch.tensor(last_air_time)
            # first_contact = torch.tensor(first_contact)
            feet_reward = torch.sum((last_air_time - 0.5) * first_contact)
            # no reward for zero command
            # print(feet_air_time)
            feet_reward = feet_reward.reshape(-1)
            feet_reward *= torch.norm(command, dim=1) > 0.1
            
            #修改处：注入数据库
            track_lin_vel_xy_exp = torch.sum(
                torch.square(torch.tensor(command[0][:2]-vel_lin_xy,device="cuda:0")),
                dim=0
            )
            # print(track_lin_vel_xy_exp)
            track_lin_vel_xy_exp = torch.exp(-track_lin_vel_xy_exp / (math.sqrt(0.25))**2)

            track_ang_vel_z_exp = torch.square(obs[0][2:3].clone().detach().to("cuda:0"))
            ang_vel_error = torch.square(command[0][2] - track_ang_vel_z_exp)
            track_ang_vel_z_exp = torch.exp(-ang_vel_error / (math.sqrt(0.25))**2)

            #print(self.env.state.imu.accelerometer[2:])
            lin_vel_z_l2 = torch.square(torch.tensor(vel_lin_z,device="cuda:0"))
            #print(vel_lin_z)
            ang_vel_xy_l2 = torch.sum(torch.square(obs[0][:2].clone().detach().to("cuda:0")), dim=0)
            dof_torques_l2 = list()
            for i in range(12):
                dof_torques_l2.append(self.env.state.motorState[i].tauEst)
            dof_torques_l2 = torch.sum(torch.square(torch.tensor(dof_torques_l2,device="cuda:0")), dim=0)
            
            action_rate_l2 = torch.sum(torch.square((action[0] - obs[0][33:]).clone().detach().to("cuda:0")), dim=0)

            #first_contact = is_contacted

            flat_orientation_l2 = torch.sum(torch.square(obs[0][3:6].clone().detach().to("cuda:0")), dim=0)  #需要四元数
            dof_pos_limits = torch.tensor([1, 1, 1 ,1, 3.5, 3.5, 3.5, 3.5, 1, 1, 1, 1],device="cuda:0") 
            
            done = 0

            rwd = track_lin_vel_xy_exp * 1.0 + track_ang_vel_z_exp * 0.5 + ang_vel_xy_l2 * (-0.05) + dof_torques_l2 * (-1.0e-5) + feet_reward * 0.125 + action_rate_l2 * -0.01
            # rwd = track_lin_vel_xy_exp * 1.0 + track_ang_vel_z_exp * 0.5 + lin_vel_z_l2 * (-2.0) + ang_vel_xy_l2 * (-0.05) + dof_torques_l2 * (-1.0e-5) + feet_reward * 0.125 + action_rate_l2 * -0.01
            # print("track_ang_vel_z_exp",track_ang_vel_z_exp)
            # print("track_lin_vel_xy_exp",track_lin_vel_xy_exp)
            # print("lin_vel_z_l2",lin_vel_z_l2)
            # print("ang_vel_xy_l2",ang_vel_xy_l2)
            # print("dof_torques_l2",dof_torques_l2)
            # print("feet_reward",feet_reward)
            # print(rwd)
            cursor.execute("INSERT INTO deployData (terrain,robot_id,obs,action,reward,done) VALUES(?,?,?,?,?,?)",(1,1,obs[0].tolist(), action[0].tolist(),rwd.item(),done))
...
'''
            ...
            # cursor.execute(
            #     """
            #     INSERT INTO deployData (terrain,robot_id,obs,action,reward,done) 
            #     VALUES(?,?,?,?,?,?)
            #     ON CONFLICT ()
            #     """,
            #     (1,1,observed_state.tolist(), control_action.tolist(),rwd,done))
            cursor.execute("INSERT INTO deployData (terrain,robot_id,obs,action,reward,done) VALUES(?,?,?,?,?,?)",(1,1,obs[0].tolist(), action[0].tolist(),rwd.item(),done))
    ...
    def robot(self, lock):
        ...
        for step in range(int(self.motor_freq*self.run_time)):
            ...
            if flag == 2 and self.env.ry>0.1: # wait for policy to generate valid command
                ...
                stand_flag_global = 1
                lock.release()
                self.env.applyAction( control_action, 10)
                self.walk_flag = 1
...
if __name__ == '__main__':
    ...
    main()
    exc_path=f"/home/zby/fall_recover/go1_deployment_2_bf/excle/{file_id}"
    if not os.path.exists(exc_path):
        os.makedirs(exc_path)

    # dof_vel_his_df  = pd.DataFrame(dof_vel_his)
    # dof_vel_his_df.to_excel(exc_path+f"/{time_id}_obs_vel.xlsx")
    # dof_pos_his_df  = pd.DataFrame(dof_pos_his)
    # dof_pos_his_df.to_excel(exc_path+f"/{time_id}_obs_pos.xlsx")
    # tau_his_df  = pd.DataFrame(tau_his)
    # tau_his_df.to_excel(exc_path+f"/{time_id}_tau.xlsx")
    # actions_no_his_df  = pd.DataFrame(actions_no_his)
    # actions_no_his_df.to_excel(exc_path+f"/{time_id}_pol_no.xlsx")
    # actions_yes_his_df  = pd.DataFrame(actions_yes_his)
    # actions_yes_his_df.to_excel(exc_path+f"/{time_id}_pol_yes.xlsx")
    # cca_his_df  = pd.DataFrame(cca_his)
    # cca_his_df.to_excel(exc_path+f"/{time_id}_pol_cca.xlsx")
