...
from unitree_guide.go1_robot_real_bf import *
...
class Actor(nn.Module): 
    ...
    def forward(self, x):
        vel,latent = self.estimator(x[0,45:])
        x = torch.cat((x[0,:45],vel,latent),dim=-1)
        return self.actor(x) 
...
class Run():
    ...
    def __init__(self, config=None, dir_path=None):
        ...
        # dir_path = self.dir_path + "/SAC/SAC/SAC/record/Go1/standup/2023_09_01_09.05.51/"#option1
        # dir_path = self.dir_path + "/SAC/SAC/SAC/record/Go1/standup/2023_08_31_13.15.48/"#option2
       ...
       # loading network 
        self.run_time = 60*10
        # print("Agent")
        # self.agent = Agent(None, self.config)
        ...
        time.sleep(5)
        # self.env._velocity_estimator.reset()
        self.env.Kp = np.array([100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100])*30.0/100.0
        self.env.Kd = np.array([3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3])*4.0/3.0#*40.0/100.0
        return
    ...
    def policy(self, lock):
        ...
        init_motor_angle =  torch.tensor(self.env._motor_angles_stand,device="cuda:0")
        for i in range(int(self.control_freq*(self.run_time+10))):
            ...
            # action = self.agent.action(state)
            # with torch.no_grad:
            obs = self.env.getObservation_him()
            action = self.actor(obs).detach()
            blend = np.clip(record_index/(self.motor_freq*1),0,1)#blend within half a sec
            action = blend*action+(1.0-blend)*init_motor_angle
            # action = obs[0,-36:-24]
    ...
    def robot(self, lock):
        ...
        while 1: #wait for button A before executing
            ...
            if ready_flag and flag==2: #button A
                break
            self.env.receiveObservation()
            # self.env.getObservation_isaacsim()
            # self.env.filterObservation()
            self.env.udp.Send()
            time.sleep(0.05)
        ...
        for step in range(int(self.motor_freq*self.run_time)):
            ...
            if self.env.button[9]: #button B
                break
            power_limit = 10#np.clip(int(step/(motor_freq/2)),1,10)#rank up power limit within half a sec
            position_limit = np.clip(step/(5*self.motor_freq),0,1)*33.5/  \
            ...
            # control_action = self.env.limitAction_stand(control_action)  # limit action in policy loop
            # self.env.filterAction(control_action)
            ...
            if flag == 2 and self.env.ry>0.1: # wait for policy to generate valid command
                ...
                stand_flag_global = 1
                lock.release()
                # self.env.applyAction_him( control_action, self.env._motor_control_mode)
                try:
                    self.env.applyAction_him( control_action, self.env._motor_control_mode)
                except:
                    lock.acquire()
