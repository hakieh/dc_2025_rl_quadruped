...
#         udpSendIntegratedPipe_0 = udpstrPrevData +  "9202" + udpstrBehindData
...
class Go1_robot():
    ...
    def __init__(self,
        ...
        self.default_pos = torch.tensor([0.1, -0.1, 0.1, -0.1, 0.6, 0.6, 1.0, 1.0, -1.5, -1.5, -1.5, -1.5 ],device= self.model_device, dtype= torch.float32).unsqueeze(0)
        ...
        self.target_vel_contact = torch.tensor([0.6 , 0, 0],device="cuda:0")
        self.power_limit = 1
        ...
        rospy.init_node("go1_real")
        self.puber=list()    #用于发布12个关节的指令，顺序为gazebo顺序
        self.puber.append(rospy.Publisher("/go1_gazebo/FL_calf_controller/command",unm.MotorCmd,queue_size=1))
        self.puber.append(rospy.Publisher("/go1_gazebo/FL_hip_controller/command",unm.MotorCmd,queue_size=1))
        self.puber.append(rospy.Publisher("/go1_gazebo/FL_thigh_controller/command",unm.MotorCmd,queue_size=1))
        self.puber.append(rospy.Publisher("/go1_gazebo/FR_calf_controller/command",unm.MotorCmd,queue_size=1))
        self.puber.append(rospy.Publisher("/go1_gazebo/FR_hip_controller/command",unm.MotorCmd,queue_size=1))
        self.puber.append(rospy.Publisher("/go1_gazebo/FR_thigh_controller/command",unm.MotorCmd,queue_size=1))
        self.puber.append(rospy.Publisher("/go1_gazebo/RL_calf_controller/command",unm.MotorCmd,queue_size=1))
        self.puber.append(rospy.Publisher("/go1_gazebo/RL_hip_controller/command",unm.MotorCmd,queue_size=1))
        self.puber.append(rospy.Publisher("/go1_gazebo/RL_thigh_controller/command",unm.MotorCmd,queue_size=1))
        self.puber.append(rospy.Publisher("/go1_gazebo/RR_calf_controller/command",unm.MotorCmd,queue_size=1))
        self.puber.append(rospy.Publisher("/go1_gazebo/RR_hip_controller/command",unm.MotorCmd,queue_size=1))
        self.puber.append(rospy.Publisher("/go1_gazebo/RR_thigh_controller/command",unm.MotorCmd,queue_size=1))

        self.puber1=list()
        self.puber1.append(rospy.Publisher("/go1_gazebo/FL_calf_controller/state",unm.MotorState,queue_size=1))
        self.puber1.append(rospy.Publisher("/go1_gazebo/FL_hip_controller/state",unm.MotorState,queue_size=1))
        self.puber1.append(rospy.Publisher("/go1_gazebo/FL_thigh_controller/state",unm.MotorState,queue_size=1))
        self.puber1.append(rospy.Publisher("/go1_gazebo/FR_calf_controller/state",unm.MotorState,queue_size=1))
        self.puber1.append(rospy.Publisher("/go1_gazebo/FR_hip_controller/state",unm.MotorState,queue_size=1))
        self.puber1.append(rospy.Publisher("/go1_gazebo/FR_thigh_controller/state",unm.MotorState,queue_size=1))
        self.puber1.append(rospy.Publisher("/go1_gazebo/RL_calf_controller/state",unm.MotorState,queue_size=1))
        self.puber1.append(rospy.Publisher("/go1_gazebo/RL_hip_controller/state",unm.MotorState,queue_size=1))
        self.puber1.append(rospy.Publisher("/go1_gazebo/RL_thigh_controller/state",unm.MotorState,queue_size=1))
        self.puber1.append(rospy.Publisher("/go1_gazebo/RR_calf_controller/state",unm.MotorState,queue_size=1))
        self.puber1.append(rospy.Publisher("/go1_gazebo/RR_hip_controller/state",unm.MotorState,queue_size=1))
        self.puber1.append(rospy.Publisher("/go1_gazebo/RR_thigh_controller/state",unm.MotorState,queue_size=1))
    ...
    def getObservation_isaacsim(self):
        ...
        # ret, frame = cmr.cap.read()
        # frame = cv2.resize(frame, (cmr.width, cmr.height))
    ...
    def getObservation_isaacsim_new(self):
        ...
        dof_vel = torch.tensor([self._raw_motor_velocities[self.dof_map_isaacsim[i]] for i in range(12)],device=self.model_device,dtype=torch.float32).unsqueeze(0)
        #print(dof_vel,"==")
        # command = torch.tensor([0.80, 0, 0],device=self.model_device,dtype=torch.float32).unsqueeze(0)
        command = self.target_vel_contact.unsqueeze(0)
    ...
    def getObservation_isaacsim_new_new(self):
        x_observation = np.zeros((self._obsDim,)) #create a new numpy array instance
        """
        Extract desired observation data from received observation
        ...
        dof_vel = torch.tensor([self._raw_motor_velocities[self.dof_map_isaacsim[i]] for i in range(12)],device=self.model_device,dtype=torch.float32).unsqueeze(0)
        #print(dof_vel,"==")
        command = torch.tensor([1.00, 0, 0.5],device=self.model_device,dtype=torch.float32).unsqueeze(0)
        contact_real=self.GetFootContacts()
        contact = torch.tensor([contact_real[1],contact_real[0],contact_real[3],contact_real[2]],device=self.model_device).unsqueeze(0)
        
        obs_step = torch.cat(  (self.base_orn_vel_base,
                                    projected_gravity,
                                    command,
                                    (dof_pos - self.default_pos),
                                    dof_vel,
    ...
    def limitAction(self, action=None, limit=None):#23.7/40.0
        ...
        delta = torch.clip(delta, -self.position_limit, self.position_limit)*1
        self.policy_action = (self._motor_angles+delta-self.default_pos)*4
        return self.policy_action
    
    def limitAction_kp_kd(self, action=None, kp=40,kd=1,limit=None):#23.7/40.0
        if action is not None:
            self.policy_action = action

        delta = self.policy_action*0.25+self.default_pos-self._motor_angles
        delta_v = torch.tensor(self._motor_velocities,device="cuda:0")
        p = torch.sum(torch.abs(delta_v)*torch.abs(delta*kp))  #-delta_v*kd
        # print(p)
        if p>900:
            limit_k = 900/p
            delta = delta*limit_k
        # pos_limit_high = (33.5+kd*delta_v)/kp
        # pos_limit_low = (-33.5+kd*delta_v)/kp
        # print("======================")
        # print("delta:",delta)
        # print("======================")
        # print("low:",pos_limit_low)
        # print("======================")
        # print("high:",pos_limit_high)
        # print("======================")
        # print("delta_v:",delta_v)
        # print("======================")
        
        # delta = torch.clip(delta, -pos_limit_low, pos_limit_high)*1

        # power limit 
        # p = torch.sum(delta * kp * self._motor_velocities)
        if limit is not None:
            self.position_limit = limit
        # delta = self.policy_action*0.25+self.default_pos-self._motor_angles
        delta = torch.clip(delta, -self.position_limit, self.position_limit)*1
    ...
    def applyAction(self, actions, power_limit=None):
        ...
        for sim_joint_idx in range(12):
            ...
            self.cmd.motorCmd[real_joint_idx].Kp = 40 #self.Kp[sim_joint_idx]#P_GAIN#Kp[motor_id]
        ...
        for i in range(12): #发布命令
            leg.motorCmd[i].mode=10
            leg.motorCmd[i].q=robot_coordinates_action[i]
            leg.motorCmd[i].dq=0
            leg.motorCmd[i].tau=0
            leg.motorCmd[i].Kp=40
            leg.motorCmd[i].Kd=1
            self.puber[dof_map[i]].publish(leg.motorCmd[i])

        m=unm.LowState()
        print(m)
        for i in range(12):
            self.puber1[dof_map[i]].publish(m.motorState[i])
    ...
    def updateTargetVel(self,tar_x=0,tar_y=0,tar_yaw=0):
        ...
        # self.target_vel_contact[0] = 1.0*float(self.ry)
        self.target_vel_contact[2] = -0.4*float(self.lx)
        # print("rx")
        return 
    ...
    def applyActionStand(self, motor_commands, motor_control_mode=None, power_limit=None):
        ...
        dof_map= [4,5,3,1,2,0,10,11,9,7,8,6]
        #print(output)   #只是为了测试
        leg=unm.LowCmd()
        for i in range(12): #发布命令
            leg.motorCmd[i].mode=10
            leg.motorCmd[i].q=motor_commands[i]
            leg.motorCmd[i].dq=0
            leg.motorCmd[i].tau=0
            leg.motorCmd[i].Kp=40
            leg.motorCmd[i].Kd=1
