...
class Run():
    ...
    def get_actor4(self):
        loaded = torch.load("./weights/model_13000.pt",map_location="cpu")
        # print(1)
        actor = alg()
        estimator = Estimator()
...
'''
    ...
    def robot(self, lock):
        ...
        for step in range(int(self.motor_freq*self.run_time)):
            ...
            self.env.receiveObservation()
            # obs = self.env.getObservation_isaacsim_new()
            observed_state = obs[0][-49:-4]
            # obs = self.env.filterObservation()
            ...
            # vel_lin_xy = np.zeros(3)
            # ang_acc = self.env.state.imu.accelerometer
            # for i in range(2):
            #     vel_lin_xy[i] += ang_acc[i] * self.motor_dt

            # last_contact_time = np.zeros(4)
            # currenttime = time.time()
            
            # contact = obs[-4:]
            
            # feet_air_time = np.zeros(4)
            # is_contacted = np.zeros(4)
            # for i in range(4):
            #     tmp = contact.tolist()[0]
            #     if tmp[i] == 1:
            #         continue
            #     else:
            #         feet_air_time[i] = currenttime - last_contact_time[i]
            #         last_contact_time[i]=currenttime
            #         if is_contacted[i] == 0:
            #             is_contacted[i] = currenttime

            # last_contact_time = torch.tensor([last_contact_time],device="cuda:0")
            # is_contacted = torch.tensor([is_contacted],device="cuda:0")

            # #注入数据库
            # track_lin_vel_xy_exp = torch.sum(
            #     torch.square(torch.tensor(vel_lin_xy,device="cuda:0")),
            #     dim=0
            # )
            # track_lin_vel_xy_exp = torch.exp(track_lin_vel_xy_exp / (math.sqrt(0.25))**2)

            # track_ang_vel_z_exp = torch.square(torch.tensor(observed_state[2:3],device="cuda:0"))
            # track_ang_vel_z_exp = torch.exp(track_ang_vel_z_exp / (math.sqrt(0.25))**2)

            # lin_vel_z_l2 = torch.square(torch.tensor(self.env.state.imu.accelerometer[2:],device="cuda:0"))
            # ang_vel_xy_l2 = torch.sum(torch.square(torch.tensor(observed_state[:2],device="cuda:0")), dim=0)
            # dof_torques_l2 = list()
            # for i in range(12):
            ...
            # action_rate_l2 = torch.sum(torch.square(torch.tensor(control_action - observed_state[33:],device="cuda:0")), dim=0)

            # first_contact = obs[0][-4:]

            # feet_reward = torch.sum((last_contact_time - torch.full((1,4),self.starttime, device="cuda:0") - 0.5) * (first_contact-torch.full((1,4),self.starttime,device = "cuda:0")), dim=1)

            # flat_orientation_l2 = torch.sum(torch.square(torch.tensor(observed_state[3:6])), dim=0)  #需要四元数
            # dof_pos_limits = torch.tensor([1, 1, 1 ,1, 3.5, 3.5, 3.5, 3.5, 1, 1, 1, 1],device="cuda:0") 
            
            # done = 0

            # rwd = track_lin_vel_xy_exp * 1.0 + track_ang_vel_z_exp * 0.5 + lin_vel_z_l2 * (-2.0) + ang_vel_xy_l2 * (-0.05) + dof_torques_l2 * (-1.0e-5) + feet_reward * 0.125 

            # #cursor.execute("INSERT INTO deployData (terrain,robot_id,obs,action,reward,done) VALUES(?,?,?,?,?,?)",(1,1,observed_state.tolist(), control_action.tolist(),rwd.item(),done))
