...
class QuadEnv(gym.Env):
    ...
    def get_obs(self):
        velocity_commands=torch.tensor(self.target,device=self.mydiv).unsqueeze(0)
    ...
    def calculate_velocity_reward(self):
        ...
        velocity_error = abs(forward_velocity - self.target[0])
    ...
    def track_lin_vel_xy_exp(self):
        lin_vel_error = (self.vel.x-self.target[0])**2 + (self.vel.y-self.target[1])**2
    ...
    def track_ang_vel_z_exp(self):
        ang_vel_error = (self.ang.z-self.target[2])**2
    ...
    def feet_air_time(self):
        ...
        reward *= np.linalg.norm(self.target[:2]) > 0.1
        return reward
    '''
    def feet_air_time(
    ...
    def compute_total_reward(self):
        ...
        dof_acc_l2 = (-2.5e-7)*self.joint_acc_l2()
        ...
        return track_lin_vel_xy_exp + track_ang_vel_z_exp + lin_vel_z_l2 + ang_vel_xy_l2 + dof_torques_l2  + dof_acc_l2 + action_rate_l2 + feet_air_time + flat_orientation_l2 + dof_pos_limits + base_height
    ...
    def step(self, acc):
        ...
        # print("0:",observation)
        reward = self.compute_total_reward()+1
    ...
    def _resample_commands(self):
        ...
        """
        import random
        self.target = [(random.random()*2-1),(random.random()*2-1),0]
        if (np.linalg.norm(self.target[:2])) < 0.2:
            self.target=[0,0,0]
