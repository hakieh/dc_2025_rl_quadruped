...
def get_encoder_script():
    ...
    depth_encoder = depth_encoder.to(torch.device("cuda"))
    depth_encoder.eval()
...
def get_actor(cfg):
    ...
    # depth_actor.load_state_dict(loaded['depth_actor_state_dict'])
    depth_actor = depth_actor.to(torch.device("cuda"))
    depth_actor.eval()

    hidden_dims_1 = [128, 64]
    estimator = Estimator(input_dim=n_proprio, output_dim=n_priv, hidden_dims=hidden_dims_1)  #to("cuda")
...
def get_actor2(cfg):
    ...
    path = "./weight/12.8/model_2420.pt"
    loaded  = torch.load(path,map_location="cpu")
    actor_critic.load_state_dict(loaded['model_state_dict'])
...
def main(args):
    ...
    depth_actor= get_actor2(config_dict) 
    
    start = rospy.get_time() 
    obs = torch.zeros((1,270)).to("cuda")
    action_test = depth_actor(obs)
    print("test_time:", rospy.get_time() - start )
    ...
    if args.mode == "upboard":
        ...
        '''
    # =============== model testing 
        # image = torch.ones(58,87).cuda().unsqueeze(0) -0.5  #debug 3.13
        # image = unitree_real_env.test().unsqueeze(0).to("cuda")
        image = np.load("image_debug.npy")
        image = torch.from_numpy(image).unsqueeze(0).cuda()
        # obs = unitree_real_env._get_proprioception_kh_obs().to(torch.float).cuda()
        obs = np.load("obs_debug.npy")
        obs = torch.from_numpy(obs).cuda().unsqueeze(0) 
        # unitree_real_env.obs_history_buf = torch.reshape(obs[:,223:].to("cpu"),(10,53))
        depth_latent_yaw = depth_encoder(image, obs[:,:n_proprio]).to("cuda")
        depth_latent = depth_latent_yaw[:,:-2]
        yaw = depth_latent_yaw[:,-2:]
        obs[:,6:8] = 1.5*yaw

        priv_predit_debug = estimator(obs[:, :n_proprio])
        # print(priv_predit_debug[0],priv_predit_debug.shape)
        # print("obs_save")
        obs[:,185:194] = priv_predit_debug
...
# ====================== testing obs kh  ===================
        image_history = torch.Tensor(np.load("./history/image_history.npy")).cuda()

        # obs_stu_history = np.load("./history/obs_stu_history.npy").cuda()
        obs_history =  torch.Tensor(np.load("./history/obs.npy")).cuda()
        '''
        ...
        with torch.no_grad():
            ...
            unitree_real_env.low_state_buffer_2 = unitree_real_env.low_state_buffer
            action_history_debug=np.zeros([1,12])  
            imu_history_debug=np.zeros([1,2])    

            flag1 = 1
            image_flag = 0
            obs_history_real=np.zeros([1,753]) 
            while not rospy.is_shutdown():
                # print("=========footForce===========",unitree_real_env.low_state_buffer.footForce)
                inference_start_time = rospy.get_time()
                finish_read = rospy.get_time() 
               ...
               # =========== image send testing ==========
                ...
                if unitree_real_env.is_skill_mode():
                    if using_walk_policy:
                        ...
                        print("switch to skill policy")
                        using_walk_policy = False
                       # model.reset()
                else:
                    if not using_walk_policy:
                        # rospy.loginfo_throttle(0.1, )
                        print("switch to walk policy")
                ...
                if not using_walk_policy:
                    ...
                    # image = unitree_real_env.test().unsqueeze(0).to("cuda")
                    # print("======= recieve image =====",image.shape)
                    obs = unitree_real_env.compute_obs_himloco().to(torch.float).to("cuda")
                    # obs_out_real = obs_real.detach().cpu().numpy()
                    # obs_history_real = np.vstack((obs_history_real,obs_out_real))
                    # obs =obs_history[flag1,:].unsqueeze(0)
                    finish_read = rospy.get_time() 
                    read_time =  finish_read - inference_start_time
                    print(read_time, "read finished")
                    # obs_last = obs
                    # obs = obs
                    # if torch.equal(obs_last[0,0:3],obs[0,0:3]):
                    #     print("simi") 
                    # else:
                    #     print("not simi")
                    ...
                    # if flag == 0:
                    #     # image_flag +=1
                    #     # image = image_history[image_flag, :].unsqueeze(0)
                    #     # image_start = rospy.get_time() 
                    #     depth_latent_yaw = depth_encoder(image, obs[:,:n_proprio]).to("cuda")
                    #     finish_iamge = rospy.get_time() 
                    #     # print(finish_iamge  - image_start,"image")
                    #     depth_latent = depth_latent_yaw[:,:-2]
                    #     yaw = depth_latent_yaw[:,-2:]
                    #     flag = (flag+1)%5
                    # else:
                    ...
                    action_start = rospy.get_time() 
                    
                    actions = depth_actor(obs).cpu()

                    finish_action = rospy.get_time() 
                    print(actions)
                    # actions[0,0] = -0.1
                    # actions[0,3] = 0.1
                    # actions[0,6] = -0.1
                    ...
                    print(finish_action - action_start, "action")

                    # # print( "=================== action computed!================")
                    # flag1 = 21
                    # if flag1 < 20:
                    #     kp = 8 + 0.5 * flag1
                    #     unitree_real_env.send_action(actions,kp)
                    # else:
                    unitree_real_env.send_action(actions)
                    '''
                    # flag1 += 1
                    # action_temp = actions_t.detach().numpy()
                    # imu_temp = obs[:,3:5].detach().cpu().numpy()
                    # # print(imu_temp)
                    # print(imu_temp,"imu_temp")
                    # imu_history_debug = np.vstack((imu_history_debug,imu_temp))
                    # action_history_debug = np.vstack((action_history_debug,action_temp))
                    # # print(len(action_history_debug))
                    # if len(action_history_debug) == 100:
                    #     np.save("action_history_debug", action_history_debug)
                    #     imu_save = pd.DataFrame(action_history_debug)
                    #     imu_save.to_excel("action.xlsx")   
                        
                    #     action_save = pd.DataFrame(imu_history_debug)
                    #     action_save.to_excel("imu.xlsx",index=False)   

                    #     action_save = pd.DataFrame(obs_history_real)
                    #     action_save.to_excel("obs_history_real.xlsx",index=False)  
                        
                    #     time_history =pd.DataFrame(unitree_real_env.time_history)
                    #     time_history.to_excel("time.xlsx",index=False)  
                    # actions =depth_actor(obs, hist_encoding=True, scandots_latent= depth_latent).cpu()
                    # actions[0:6] = actions[0:6]*0.5
                    # actions[6:] = actions[6:]*0.3
                    '''
                    
                    inference_duration = rospy.get_time() - inference_start_time                   
                    print(inference_duration, "infer")
                   ...
                   # print("=========== standing skill")
                    ...
                    unitree_real_env.send_action(actions, 35)
