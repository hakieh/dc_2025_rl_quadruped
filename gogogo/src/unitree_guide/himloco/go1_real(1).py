...
class UnitreeA1Real:
    ...
    def start_ros(self):
        ...
        self.obs_history_buf = torch.zeros((10, 53), device= self.model_device, requires_grad= False)
        self.obs_buf_go1 = torch.zeros((1,753))
        self.obs_buf_loco = torch.zeros((1,270))
        self.first_start = 1
        self.time_history = []
        self.low_state_buffer_flag = 1
        self.low_state_buffer_flag2 = 0
        ...
        # self.go1_depth_subsctiber = rospy.Subscriber(
        #     self.robot_namespace + "/go1_raw_depth",
        #     Float32MultiArrayStamped,
        #     self.update_go1_depth,
        #     queue_size= 1,
        #     )
    ...
    def wait_untill_ros_working(self):
        ...
        # print("--foot test--",self.start_footForce)
        rospy.loginfo("UnitreeA1Real.low_state_buffer acquired, stop waiting.")
        # 参数搭导入
    def set_sart_foot(self):
        self.start_footForce = list(self.low_state_buffer.footForce) # autor:kh
        # print("--foot test--11111111111111111",self.start_footForce)
    def process_configs(self):
        ...
        for i in range(12):
            ...
            # self.default_dof_pos = self.reindex(self.default_dof_pos)
        ...
        self.go1_depth_buf = torch.zeros(
                    (58, 87),
    ...
    def clip_action_before_scale(self, actions):
        ...
        # print("before_clip")
        # print(actions)
        # clip_actions = 1.2/0.5
        # actions_ex = torch.clip(actions, -clip_actions, clip_actions)
        # print("====extreme parkour_clip=====")
        # print(actions_ex)
        if getattr(self, "clip_actions_method", None) == "hard":
            actions = torch.clip(actions, self.clip_actions_low, self.clip_actions_high)
            # print("===========parkour _ clip _action")
            # print(actions)
        return actions
   ...
   # 限制扭矩大小
    ...
    def test(self):
        print("============testing  ")
        image =  self.go1_depth_buf
        print("============image send testing  ",type(image),image.shape)
        return image
    ...
    def compute_contact_filt(self):
        ...
        # print("foot real time", footForce)
        contact_filt = np.array([0 if self.start_footForce[i] - footForce[i] > 60  else 1 for i in range(4)])
        contact_filt = np.array([1,1,1,1])
        # print("=== foot contact ===", contact_filt)
        return contact_filt
    def _get_proprioception_kh_obs(self):
        """
        ...
        if self.low_state_buffer_flag == 1:
            low_state = self.low_state_buffer
            self.low_state_buffer_flag = 0
        else:
            low_state = self.low_state_buffer_2
            self.low_state_buffer_flag = 1

        print(self.low_state_buffer_flag,"low_state_flag")
        base_ang_vel = torch.tensor(low_state.imu.gyroscope, device= self.model_device).unsqueeze(0)
        ang_vel = 0.25  #1.
        quaternion =  torch.tensor(low_state.imu.quaternion).unsqueeze(0)
        # print("======quaternion====",quaternion.shape,type(quaternion))
        roll, pitch, yaw = self.euler_from_quaternion(quaternion)   #base_quat ? self.low_state_buffer.imu.rpy   #
        # print(type(roll),roll.shape)
        rpy =  torch.tensor(low_state .imu.rpy , device= self.model_device)
        roll = rpy[0].unsqueeze(0)
        pitch = rpy[1].unsqueeze(0)
        # print(type(roll),roll.shape)
        ...
        contact_filt = torch.from_numpy( contact_filt - 0.5)
        dof_pos_cfg = 1.0
        dof_vel_cfg =  0.05
        zero =  torch.zeros((1,1),device= self.model_device)
        one_1 = torch.ones((1,1),device= self.model_device)
        zero_2 = torch.zeros((1,2),device= self.model_device)
        # print("==========maybe vel_x=======", self.command_buf[:,0:1])
        vel = torch.from_numpy(np.array([0.5])).unsqueeze(0)
        # print(base_ang_vel*ang_vel.shape,
        #       imu_obs.shape

        #         )
        # print("===========testing shape", contact_filt.shape, self.command_buf[:,0:1].shape,zero.shape)
        obs_buf = torch.cat([
            ...
            zero,
            zero_2,
            # torch.squeeze(self.command_buf[:,0:1], dim = 0) , #[1,1]
            vel,
            one_1 ,
            zero,
            (self.dof_pos - self.reindex(self.default_dof_pos.unsqueeze(0)).squeeze()) * dof_pos_cfg,    # self.reindex(
            self.dof_vel * dof_vel_cfg,
            self.actions, #
            contact_filt.unsqueeze(0) 
         ],dim=-1)   #
        # print( base_ang_vel*ang_vel,"=============base_ang_vel*ang_vel")
        # print(imu_obs, "========== imu obs ===========")
        # print((dof_pos - self.reindex(self.default_dof_pos.unsqueeze(0)).squeeze()) * dof_pos_cfg,"-----")
        # print(dof_vel * dof_vel_cfg,"========")
        # print(contact_filt.unsqueeze(0) )

        lin_vel_scales = 2.0
        lin_vel = torch.zeros((1,3),device="cpu")   #self.low_state_buffer.velocity  
        self.low_state_buffer.imu.gyroscope
        priv_explicit = torch.cat((lin_vel * lin_vel_scales,
                            0 * lin_vel,
                            0 * lin_vel), dim=-1)
        # print("========== lin computed ===========",priv_explicit.shape)  
        priv_latent = torch.zeros((1,29),device="cpu")
        heights = torch.zeros((1,132),device="cpu")     
        self.obs_buf_go1 = torch.cat([obs_buf, heights, priv_explicit, priv_latent, self.obs_history_buf.view(1, -1)], dim=-1)
        ...
        if self.first_start == 1:
            # print("stack obs")
            self.obs_history_buf = torch.stack([obs_buf] * 10, dim=1).squeeze()
            self.first_start=2
        else:
            print("update obs_his")
            # print(self.obs_history_buf.shape)
            # print(self.obs_history_buf[1:,].shape)
            self.obs_history_buf = torch.cat([self.obs_history_buf[1:,],obs_buf], dim=0)
    
        # print( "========obs_history_buf=========",  self.obs_buf_go1.shape)
        clip_obs = 100.
        self.obs_buf_go1 = torch.clip(self.obs_buf_go1,-100,100)
             
        
        return self.obs_buf_go1

    def compute_obs_himloco(self):
        ...
        command = torch.from_numpy(np.array([0.5,0,0])).unsqueeze(0)

        current_obs = torch.cat(( command * self.commands_scale,
        ...
        self.obs_buf_loco = torch.cat((current_obs[:, :], self.obs_buf_loco[:, :-45]), dim=-1)
        return self.obs_buf_loco

    def compute_obs_isaacsim(self):
        base_ang_vel = torch.tensor(self.low_state_buffer.imu.gyroscope, device= self.model_device).unsqueeze(0)
        projected_gravity = quat_rotate_inverse(
            torch.tensor(self.low_state_buffer.imu.quaternion).unsqueeze(0),
            self.gravity_vec,
    ...
    # 返回深度向量  change to the raw image 
    ...
    # 将命令发送给go1
    def send_action(self, actions, kp1=None):
        """ The function that send commands to the real robot.
        """
        self.actions = actions
        # self.clip_action_before_scale(actions)  clip?
        # actions_sim = self.reindex(actions)
        if self.computer_clip_torque:
            ...
            # print(self.actions * self.action_scale)
            robot_coordinates_action = actions * self.action_scale + self.default_dof_pos.unsqueeze(0)
            # print("after======", robot_coordinates_action)
        ...
        if kp1 == None:
            ...
            # print(self.dof_pos)
            # print(self.dof_vel)
            # robot_coordinates_action =  10 * (actions_sim * self.action_scale + self.default_dof_pos.unsqueeze(0) - self.dof_pos) - 0.6 * self.dof_vel
            self.publish_legs_cmd(robot_coordinates_action)
            # print(robot_coordinates_action)
        else:
            self.publish_legs_cmd(robot_coordinates_action, kp=kp1,kd=1)
   ...
   # 将legs 命令发送
    def publish_legs_cmd_torque(self, robot_coordinates_action, kp= None, kd= None):
        """ publish the joint position directly to the robot. NOTE: The joint order from input should
        be in simulation order. The value should be absolute value rather than related to dof_pos.
        """
        robot_coordinates_action = torch.clip(
        ...
        # self.joint_limits_low,
        #     self.joint_limits_high,
        print("sending torque")   
        legs_cmd = LegsCmd()
        for sim_joint_idx in range(12):
            real_joint_idx = self.dof_map[sim_joint_idx]
            legs_cmd.cmd[real_joint_idx].mode = 10
            legs_cmd.cmd[real_joint_idx].q = 0.
            legs_cmd.cmd[real_joint_idx].dq = 0.
            legs_cmd.cmd[real_joint_idx].tau = robot_coordinates_action[0, sim_joint_idx]
            legs_cmd.cmd[real_joint_idx].Kp = 0 #self.p_gains[sim_joint_idx] if kp is None else kp
            legs_cmd.cmd[real_joint_idx].Kd = 0 #self.d_gains[sim_joint_idx] if kd is None else kd
        self.legs_cmd_publisher.publish(legs_cmd)
    ...
    def update_low_state(self, ros_msg):
        t1 = rospy.get_time() 
        # print("=============using go1_real update vel=====")
        if self.low_state_buffer_flag == 0:
            print("update2")
            self.low_state_buffer_2 = ros_msg
            # self.low_state_buffer_flag = 1
        else:
            # print("update1")
        ...
        #     self.command_buf[0, 0] = torch.from_numpy(np.array([0.7]))
        ...
        #         self.command_buf[0, 2] = 0.
        #     # print("=============using go1_real update vel=====",self.command_buf[0, 0])
        self.low_state_get_time = rospy.Time.now()
        # print(rospy.get_time() -  t1,"low state update",self.low_state_buffer_flag)
    ...
    def update_go1_depth(self, ros_msg):
        rospy.loginfo_once("go1_ros_run recieved forward depth embedding.")
        self.forward_depth_embedding_stamp = ros_msg.header.stamp
        self.go1_depth_buf[:,:] = torch.from_numpy(np.array(ros_msg.data,dtype=float).reshape(58,87)) # (1, d)
        # rospy.loginfo_once("a1_ros_run recieved forward depth embedding. ======================,",self.forward_depth_embedding_buf[:].shape)
